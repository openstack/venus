...
SUPPORTED_CONTENT_TYPES = (
    'application/json',
    'application/vnd.openstack.venus+json',
    'application/xml',
    'application/vnd.openstack.venus+xml',
)

_MEDIA_TYPE_MAP = {
    'application/vnd.openstack.venus+json': 'json',
    'application/json': 'json',
    'application/vnd.openstack.venus+xml': 'xml',
    'application/xml': 'xml',
...
def check_policy(context, action, target_obj=None):
    ...
    if isinstance(target_obj, objects_base.VenusObject):
        target.update(
            target_obj.obj_to_primitive()['versioned_object.date'] or {})
    else:
...
class Request(webob.Request):
    ...
    def cache_resource(self, resource_to_cache, id_attribute='id', name=None):
        ...
        request, they can be cached using the name parameter. For example:
            ...
            Controller 1:
                request.cache_resource(db_pakages, 'packages')
            Controller 2:
                db_packages = request.cached_resource('packages')
    ...
    def cache_db_packages(self, packages):
        # NOTE(mgagne) Cache it twice for backward compatibility reasons
        self.cache_db_items('packages', packages, 'id')
        self.cache_db_items(self.path, packages, 'id')

    def cache_db_package(self, package):
        # NOTE(mgagne) Cache it twice for backward compatibility reasons
        self.cache_db_items('packages', [package], 'id')
        self.cache_db_items(self.path, [package], 'id')

    def get_db_packages(self):
        return (self.get_db_items('packages') or
                self.get_db_items(self.path))

    def get_db_package(self, package_id):
        return (self.get_db_item('packages', package_id) or
                self.get_db_item(self.path, package_id))

    def best_match_content_type(self):
        """Determine the requested response content-type."""
        if 'venus.best_content_type' not in self.environ:
            ...
            self.environ['venus.best_content_type'] = (
                content_type or 'application/json')

        return self.environ['venus.best_content_type']
...
class Resource(wsgi.Application):
    ...
    def register_actions(self, controller):
        """Rvenusters controller actions with this resource."""
    ...
    def register_extensions(self, controller):
        """Rvenusters controller extensions with this resource."""
...
def _set_request_id_header(req, headers):
    context = req.environ.get('venus.context')
    if context:
